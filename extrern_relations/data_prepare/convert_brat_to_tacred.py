    ...
    for annotation_filename in annotations:
        ...
        for line in lines:
            if not line.strip():
                flag += len(line)
                continue
            line_sents = list(razdel.sentenize(line))
            #print(line)
            #print('\\\\\\\\\\\\\\\\')
            #print(line_sents)
            #print()
            #print('------------------------------')
            for i in range(len(line_sents)):
                line_sents[i].start += flag
                line_sents[i].stop += flag
            flag += len(line)
...
def main(path: Optional[str] = None):
    ...
    """
    path = "/home/alex/WORK/Lab/14_02/filter_data/all/*.ann"
    #path = '/home/alex/WORK/Lab/20_12/filter2_data/064.ann'
    #path = '/home/alex/WORK/Lab/20_12/filter2_data/114619_text.ann'
    #path = '/home/alex/WORK/Lab/20_12/filter2_data/1109.ann'
    if path:
    ...
    all_data: List[Dict] = []
    for k, v in tqdm(text_data.items()):
        for sent_i, sent in enumerate(v):
            sent["filename"] = k
        all_data += list(v)

    with open(f"all_data.json", "w") as f:
    #with open(f"tmp_data.json", "w") as f:
